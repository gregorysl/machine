...
New-Item -Path $localPackagesSource -ItemType Directory -Force | Out-Null
...
choco pack "$PSScriptRoot\semanticmerge.install\semanticmerge.install.nuspec" -f -out $localPackagesSource
...
choco upgrade --cache="$ChocoCachePath" --yes semanticmerge.install -s $localPackagesSource
